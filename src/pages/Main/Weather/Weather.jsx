...
export const Weather = ({
  ...
  if (!city && favoriteCities.length === 0) return null;
  ...
  const handleLikeClick = (cityWeather) => {
    const isFavorite = favoriteCities.some((c) => c.id === cityWeather.id);
    let updatedFavorites;

    if (isFavorite) {
      updatedFavorites = favoriteCities.filter((c) => c.id !== cityWeather.id);
      setSearchedCities((prev) => [...prev, cityWeather]);
      addNotification("City removed from favorites.", "info");
    } else {
      updatedFavorites = [...favoriteCities, cityWeather];
      setSearchedCities((prev) => prev.filter((c) => c.id !== cityWeather.id));
        ...
        {!loading &&
          ...
          favoriteCities.length === 0 && (
            <div className={style.weather__empty}>
              <p>
                {isLoggined
                  ? "Welcome back! Search for a city to see weather information or add cities to favorites."
                  : "No cities found yet. Search for a city to see weather information."}
              </p>
            </div>
          )}
        {!loading &&
          (searchedCities.length > 0 || favoriteCities.length > 0) && (
